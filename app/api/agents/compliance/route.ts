...
export async function POST(req: Request) {
  try {
    ...
    const stream = createUIMessageStream({
      execute: ({ writer: dataStream }) => {
        const result = agent.generate(messages, {
          ...
          onFinish: async ({ text, toolCalls, usage, finishReason }) => {
            console.log("[Compliance Agent] Finished processing:", {
              userId: user.id,
            ...
            try {
              ...
              console.log(
                `[Compliance Agent] Saved ${dbMessages.length} messages`
              );
            } catch (error) {
              console.error(
                "[Compliance Agent] Failed to save messages:",
                error
              );
            }
          },
      ...
      onError: (error) => {
        console.error("[Compliance Agent] Stream error:", error);
