...
export const xeroAccountCache = pgTable(
  ...
  (table) => [
    uniqueIndex("xero_account_cache_unique").using(
      "btree",
...
export const xeroBankTransactionCache = pgTable(
  ...
  (table) => [
    uniqueIndex("xero_bank_transaction_cache_unique").using(
      "btree",
      table.tenantId.asc().nullsLast().op("text_ops"),
      table.bankTransactionId.asc().nullsLast().op("text_ops")
    ),
  ]
);
...
export const xeroContactCache = pgTable(
  ...
  (table) => [
    uniqueIndex("xero_contact_cache_unique").using(
      "btree",
      table.tenantId.asc().nullsLast().op("text_ops"),
...
export const xeroInvoiceCache = pgTable(
  ...
  (table) => [
    uniqueIndex("xero_invoice_cache_unique").using(
      "btree",
      table.tenantId.asc().nullsLast().op("text_ops"),
...
export const chatXeroContext = pgTable(
  ...
  (table) => [
    uniqueIndex("chat_xero_context_chat_unique").using(
      "btree",
...
export const regulatoryDocument = pgTable(
  ...
  (table) => [
    index("regulatory_document_category_idx").using(
      "btree",
      table.category.asc().nullsLast().op("text_ops")
    ),
    index("regulatory_document_country_idx").using(
      "btree",
      table.country.asc().nullsLast().op("text_ops")
    ),
    index("regulatory_document_source_url_idx").using(
      "btree",
      table.sourceUrl.asc().nullsLast().op("text_ops")
    ),
    index("regulatory_document_status_idx").using(
      "btree",
...
export const xeroConnection = pgTable(
  ...
  (table) => [
    uniqueIndex("xero_connection_id_idx")
      .using("btree", table.connectionId.asc().nullsLast().op("text_ops"))
      .where(sql`("connectionId" IS NOT NULL)`),
    uniqueIndex("xero_connection_user_tenant_idx").using(
      "btree",
...
export const regulatoryScrapeJob = pgTable(
  ...
  (table) => [
    index("regulatory_scrape_job_country_idx").using(
      "btree",
      table.country.asc().nullsLast().op("text_ops")
    ),
    index("regulatory_scrape_job_created_at_idx").using(
      "btree",
      table.createdAt.asc().nullsLast().op("timestamp_ops")
    ),
    index("regulatory_scrape_job_status_idx").using(
      "btree",
      table.status.asc().nullsLast().op("text_ops")
    ),
  ]
...
export const qaReviewRequest = pgTable(
  ...
  (table) => [
    index("qa_review_request_status_idx").using(
      "btree",
      table.status.asc().nullsLast().op("text_ops")
    ),
    index("qa_review_request_user_id_idx").using(
      "btree",
