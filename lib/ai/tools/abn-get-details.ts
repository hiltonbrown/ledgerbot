...
export const abn_get_details = tool({
  ...
  execute: async ({ identifier, kind }) => {
    ...
    const effectiveKind =
      kind === "AUTO" || kind === classification.kind
        ? classification.kind
        : kind;
    ...
    if (effectiveKind === "ACN") {
      return {
        error: {
          ...
          message:
            "Direct ACN lookup via ABR not fully supported. Please search by name or ABN.",
        },
      };
    }
    ...
    try {
      ...
      if (result.results.length === 0) {
        return {
          error: { code: "NOT_FOUND", message: "No entity found for this ABN" },
        };
      }
