...
async function fetchXeroContact(
  userId: string,
  contactId: string
    ...
    execute: async ({ xeroContactId, asOfDate }) => {
      ...
      if (!contact) {
        return {
          ...
          message: "Contact not found in Xero",
        };
      }
      ...
      const entityNameMatch = isEntityNameMatch(
        contact.name,
