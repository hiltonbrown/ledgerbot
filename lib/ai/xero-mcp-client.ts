...
async function getXeroClient(
	userId: string
): Promise<{ client: AccountingAPIClient; connection: DecryptedXeroConnection }> {
	...
	await client.setAccessToken(connection.accessToken);
...
export const xeroMCPTools: XeroMCPTool[] = [
	{
		...
		inputSchema: {
			...
			properties: {
				status: {
					type: "string",
					description: "Invoice status filter (DRAFT, SUBMITTED, AUTHORISED, PAID, VOIDED)",
	...
	},
	{
		...
		inputSchema: {
			...
			properties: {
				type: {
					type: "string",
					description: "Filter by account type (BANK, CURRENT, EXPENSE, FIXED, etc.)",
				},
			},
		},
	},
	...
	try {
		...
		switch (toolName) {
			case "xero_list_invoices": {
				...
				const response = await client.getInvoices(connection.tenantId, undefined, where, undefined, undefined, undefined, undefined, undefined, limit as number);
			...
			case "xero_get_invoice": {
				...
				const response = await client.getInvoice(connection.tenantId, invoiceId as string);
			...
			case "xero_list_contacts": {
				...
				const where = searchTerm ? `Name.Contains("${searchTerm}") OR EmailAddress.Contains("${searchTerm}")` : undefined;

				const response = await client.getContacts(connection.tenantId, undefined, where, undefined, undefined, undefined, undefined, limit as number);
			...
			case "xero_get_contact": {
				...
				const response = await client.getContact(connection.tenantId, contactId as string);
			...
			case "xero_list_accounts": {
				...
				const response = await client.getAccounts(connection.tenantId, undefined, where);
			...
			case "xero_list_journal_entries": {
				...
				const response = await client.getManualJournals(
					connection.tenantId,
					undefined,
					undefined,
					undefined,
				...
				if (dateFrom || dateTo) {
					journals = journals.filter((journal) => {
						...
						if (dateFrom && journalDate < new Date(dateFrom as string)) return false;
						if (dateTo && journalDate > new Date(dateTo as string)) return false;
						return true;
					});
				}
			...
			case "xero_get_bank_transactions": {
				...
				const whereClauses: string[] = [];
				if (bankAccountId) whereClauses.push(`BankAccount.AccountID==Guid("${bankAccountId}")`);
				if (dateFrom) whereClauses.push(`Date>=DateTime(${dateFrom})`);
				if (dateTo) whereClauses.push(`Date<=DateTime(${dateTo})`);

				const where = whereClauses.length > 0 ? whereClauses.join(" AND ") : undefined;

				const response = await client.getBankTransactions(
					connection.tenantId,
					undefined,
					where,
					undefined,
			...
			case "xero_get_organisation": {
				const response = await client.getOrganisations(connection.tenantId);
